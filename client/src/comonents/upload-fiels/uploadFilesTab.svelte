		...
		if (!token) {
			...
			try {
				...
				if (response.ok) {
					...
					notifier.success('הקובץ הועלה בהצלחה');
					formElement.reset();
				} else {
					console.error('HTTP error:', response.status, response.statusText);
					notifier.danger('שגיאה בהעלאת קובץ ' + response.statusText);
				}
			} catch (error) {
				formElement.reset();
				console.error('Error uploading files:', error);
				notifier.danger('שגיאה בהעלאת קובץ ' + error);
			}
		}
	}
	onMount(async () => {
		...
		{#if files}
			{#each files as file}
				...
				<div class="link-container">
					...
					{#if isCLicked === file}
						<EditComponent {file} {project_id} on:updatedData={getDataFromEdit} />
					{:else}
						<div class="div1" />
					{/if}
				</div>
