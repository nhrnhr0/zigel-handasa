<script>
	import RelatedDocsTable from '../../../layout/RelatedDocsTable.svelte';
	import ToolTipComponent from '../../../shered/ToolTipComponent.svelte';

	export let data;
	let related_docs = data?.row?.related_docs ?? [];
	let doc_number = data?.row?.doc_number ?? '';
	let has_related_docs = related_docs?.parents.length > 0 || related_docs?.childs.length > 0;
</script>

<!-- <pre>{JSON.stringify(data)}</pre> -->
<td>
	<ToolTipComponent bind:active={has_related_docs}>
