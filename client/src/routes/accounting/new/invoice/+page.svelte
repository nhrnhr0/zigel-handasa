<script>
	...
	function craete_invoice_from_price_proposals(docs) {
		...
		for (let i = 0; i < docs.length; i++) {
			let doc = docs[i];
			if (doc.related_to.length > 0) {
				// TODO: add an item with the total of the price proposal minus the related_to invoices totals
			} else {
				// add the items from the price proposal
